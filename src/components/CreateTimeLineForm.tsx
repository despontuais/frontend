...
const CreateTimelineForm: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const addMedia = async () => {
    try {
      const response = await axios.post('https://cronolog.duckdns.org/api/media/createMedia', {
        title: mediaTitle,
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      await axios.post('https://cronolog.duckdns.org/api/timeline/createTimeline', {
        title: timelineTitle,
